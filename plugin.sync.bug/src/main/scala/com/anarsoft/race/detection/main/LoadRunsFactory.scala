...
import com.anarsoft.race.detection.event.loadAndDistribute.LoadAndDistributeOneFilePositionImpl.syncAction
...
import com.vmlens.trace.agent.bootstrap.event.StreamRepository.SYNC_ACTIONS
...
class LoadRunsFactory {
  ...
  def create(dir: Path): LoadRuns = {
    ...
    arrayBuffer.append(create(dir, SYNC_ACTIONS,
      (path, name) => syncAction(path, name)));
