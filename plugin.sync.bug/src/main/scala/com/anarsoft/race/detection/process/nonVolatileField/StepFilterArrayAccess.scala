...
class StepFilterArrayAccess  extends SingleStep[ContextNonVolatileFields] {
  ...
  def execute(contextNonVolatileFields: ContextNonVolatileFields) {
    ...
    contextNonVolatileFields.arrayAccessEventList = filter(    contextNonVolatileFields.arrayAccessEventList ,   -1L);
