...
import com.vmlens.report.assertion.{DebugLogger, DebugLoggerToSystemOut, OnDescriptionAndEvent, OnDescriptionAndEventNoOp}

import java.nio.file.{Path, Paths}


class DebugEvents(val dir : Path, val onDescriptionAndEvent : OnDescriptionAndEvent,val debugLogger: DebugLogger) {
  ...
  def process(): Unit = {
    ...
    val runDataListBuilderForDebug = new RunDataListBuilderForDebug(debugLogger,onDescriptionAndEvent);
...
object DebugEvents {
  ...
  def main(args: Array[String]): Unit = {
    ...
    new DebugEvents(dir,new OnDescriptionAndEventNoOp(),new DebugLoggerToSystemOut()).process();
  }
